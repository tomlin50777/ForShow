...
namespace BoxChartProject
{
	public partial class Form1 : Form
	{
		...
		private void button1_Click(object sender, EventArgs e)
		{
			...
			{
				if(count ==0)
					boxChartControl1.AddData(fakeData.Select("tagName = 'Tag_" + count + "'").CopyToDataTable(), "date", "value", colors[count], -2, 3100, 0F);
				else
					boxChartControl1.AddData(fakeData.Select("tagName = 'Tag_" + count + "'").CopyToDataTable(), "date", "value", colors[count], -2, 3100, 2F);
			}
		}
		/// <summary>
		/// Faks Data製作用，
