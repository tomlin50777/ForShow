...
namespace BoxChartProject
{
	public partial class BoxChartControl : UserControl
	{
		...
		/// <param name="yFix">修正值，用來壓縮沒有Axis圖的高度</param>
		public void AddData(DataTable dataTable, String xAxisColumnName, String valueColumnName, Color color, double yMax, double yMin, Single xFix, Single yFix)
		{
			DateTime startDate;
			DateTime endDate;
			...
			BoxChart newBoxChart = AddItem(color, yMin, yMax, xFix, yFix);
		...
		/// <returns></returns>
		private BoxChart AddItem(Color color, double max, double min, Single xFix, Single yFix)
		{
			BoxChart boxChart = new BoxChart(color, max, min);
			TableLayoutPanel tableLayoutPanel = NewTableLayoutPanel(yFix);
			...
			{
				if (count == 0)
					chartTableLayoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, blockWidth + xFix * (chartTableLayoutPanel.ColumnCount - 1)));
				else
					chartTableLayoutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, blockWidth + xFix));
			}
		...
		private TableLayoutPanel NewTableLayoutPanel(Single yFix)
		{
			...
			tableLayoutPanel.RowStyles.Add(new RowStyle(SizeType.Percent, yFix));
			tableLayoutPanel.RowStyles.Add(new RowStyle(SizeType.Percent, 100F - yFix));
